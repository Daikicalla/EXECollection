...
class PlayState extends MusicBeatState
{
	...
	*/
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'carnival-night':
				...
				var back = new BGSprite('requite/back', 1700, -1300);
				back.scale.set(2.5, 2.5);
				lightsup.add(back);

				var pads = new BGSprite('requite/pads', 1000, -700);
				pads.scale.set(2.7, 2.7);
				lightsup.add(pads);

				var pedestal = new BGSprite('requite/pedestal', 1180, 500);
				pedestal.scale.set(1.9, 1.9);
				lightsup.add(pedestal);

				var floor = new BGSprite('requite/floor', -1100, 900);
		...
		if (sonicHUDStyles.exists(SONG.song.toLowerCase()))
			hudStyle = sonicHUDStyles.get(SONG.song.toLowerCase());
		...
		if (SONG.song.toLowerCase() == 'you-cant-run' || SONG.song.toLowerCase() == 'you-cant-run-encore')
		{
					...
					new FlxTimer().start(0.5, function(lol:FlxTimer)
					{
						if (true)
						{
							new FlxTimer().start(1, function(lol:FlxTimer)
							{
								...
								FlxG.sound.play(Paths.sound('intruder', 'exe'));
								FlxG.camera.flash(FlxColor.RED, 0.2);
							});
							new FlxTimer().start(2, function(lol:FlxTimer)
							{
								FlxG.sound.play(Paths.sound('sonicfall', 'exe'));
	...
	public function startCountdown():Void
	{
		...
		if (curSong.toLowerCase() == 'forestall-desire')
		{
			extrachar = new Boyfriend(120, 200, 'pico-requital');
			extrachar.flipX = false;
