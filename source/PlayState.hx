...
class PlayState extends MusicBeatState
{
	...
	public var EXTRACHAR_X:Float = 50;
	public var EXTRACHAR_Y:Float = 50;
	...
	public var extracharGroup:FlxSpriteGroup;
	...
	public static var isFreeplayMode:Bool = false;
	...
	public var extrachar:Boyfriend = null;
	...
	// -- The var's for gameplay -- //
	// Phase 1 Sonic
	var fgTrees:BGSprite;
	// Phase 2 Sonic
	var greenhills:FlxSprite;
	var ycrbg:FlxTypedGroup<FlxSprite>;
	// Fleetway
	var wall:FlxSprite;
	var floor:FlxSprite;
	...
	// Requital Stuff (OH MY GOD!!)
	var lightsup = new FlxTypedGroup<FlxSprite>();
	var lightsdown = new FlxTypedGroup<FlxSprite>();
	...
	// Health stuff mechanics or whatever
	...
	override public function create()
	{
		...
		if(SONG.stage == null || SONG.stage.length < 1) {
			switch (songName)
			{
				case 'too-slow':
					curStage = 'too-slow';
				case 'you-cant-run':
					curStage = 'cant-run-xd';
				// case 'you-cant-run-encore':
					// curStage = 'SonicP2Encore';
				default:
					curStage = 'stage';
			}
		}
		...
		EXTRACHAR_X = stageData.extrachar[0];
		EXTRACHAR_Y = stageData.extrachar[1];
		...
		extracharGroup = new FlxSpriteGroup(EXTRACHAR_X, EXTRACHAR_Y);
		...
		switch (curStage)
		{
			...
			case 'cant-run-xd': // Wooooow
				ycrbg = new FlxTypedGroup<FlxSprite>();
				...
				ycrbg.add(treesfront);

				var topoverlay:BGSprite = new BGSprite('run/TopOverlay', -600, -200, 1.0, 1.0);
				ycrbg.add(topoverlay);

				add(ycrbg);
			
			case 'SonicP2Encore':
				var ycrencorebg = new FlxTypedGroup<FlxSprite>();
				var greenhillsbg = new FlxTypedGroup<FlxSprite>();

				var sky:BGSprite = new BGSprite('run-encore/sky', -700, -2000);
				sky.scale.set(3.8, 3.8);
				ycrencorebg.add(sky);

				var backbg:BGSprite = new BGSprite('run-encore/backBG', -700, -300);
				backbg.scrollFactor.set(0.9, 0.9);
				backbg.scale.set(1.3, 1.3);
				ycrencorebg.add(backbg);

				var backfront:BGSprite = new BGSprite('run-encore/backFront', -700, -100);
				backfront.scrollFactor.set(0.9, 0.9);
				backfront.scale.set(1.3, 1.3);
				ycrencorebg.add(backfront);

				var clouds:BGSprite = new BGSprite('run-encore/clouds', -700, 200);
				clouds.scrollFactor.set(0.9, 0.9);
				clouds.scale.set(1.3, 1.3);
				ycrencorebg.add(clouds);

				var frontbg:BGSprite = new BGSprite('run-encore/frontBG', -700, 50);
				frontbg.scrollFactor.set(0.9, 0.9);
				frontbg.scale.set(1.3, 1.3);
				ycrencorebg.add(frontbg);

				var floor:BGSprite = new BGSprite('run-encore/floor', -1000, 650);
				floor.scale.set(1.1, 1.1);
				ycrencorebg.add(floor);
			...
			case 'carnival-night':
				var sky = new BGSprite('requite/sky', -1300, -980);
				sky.scale.set(1.7, 1.7);
				lightsup.add(sky);

				var mountain2 = new BGSprite('requite/mountain2', -1900, 0);
				mountain2.scale.set(0.5, 0.4);
				lightsup.add(mountain2);

				var mountain1 = new BGSprite('requite/mountain1', -1300, 100);
				mountain1.scale.set(0.5, 0.5);
				lightsup.add(mountain1);

				var stars = new BGSprite('requite/stars', -50, -400);
				stars.scale.set(1.8, 1.8);
				lightsup.add(stars);

				var back = new BGSprite('requite/back', 2200, 200);
				back.scale.set(2.5, 2.5);
				lightsup.add(back);

				var pads = new BGSprite('requite/pads', 850, -350);
				pads.scale.set(2.7, 2.7);
				lightsup.add(pads);

				var pedestal = new BGSprite('requite/pedestal', 1400, 830);
				pedestal.scale.set(1.9, 1.9);
				lightsup.add(pedestal);

				var floor = new BGSprite('requite/floor', -700, 1050);
				floor.scale.set(1.7, 1.7);
				lightsup.add(floor);

				var idunno = new BGSprite('requite/idunno', -1200, -1300);
				idunno.scale.set(1.5, 1.8);
				lightsup.add(idunno);
		...
		switch(curStage)
		{
			case 'starved-pixel' | 'chamber' | 'carnival-night':
		...
		var extraVersion:String = SONG.extraVersion;
		if (extraVersion == null || extraVersion.length < 1)
		{
			switch (curStage)
			{
				default:
					extraVersion = 'boyfriend';
			}
			SONG.extraVersion = extraVersion;
		}
		...
		if (!stageData.hide_extrachar)
		{
			extrachar = new Boyfriend(0, 0, extraVersion);
			startCharacterPos(extrachar);
			extracharGroup.add(extrachar);
	...
	public function startCountdown():Void
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				startTimer = new FlxTimer().start(Conductor.crochet / 1000, function(tmr:FlxTimer)
				{
					...
					if (extrachar != null && tmr.loopsLeft % extrachar.danceEveryNumBeats == 0 && extrachar.animation.curAnim != null && !extrachar.animation.curAnim.name.startsWith('sing') && !extrachar.stunned)
					{
						extrachar.dance();
					}
	...
	override public function update(elapsed:Float)
	{
		...
		switch (flyState)
		{
			...
			// case 'flying' | 'flying': "Warning: This pattern is unused" i hate you HaxeFlixel
				// flyTarg.y += Math.sin(floaty) * 1.5;
		...
		if (generatedMusic)
		{
			if (!inCutscene) {
				...
				else if(extrachar != null && extrachar.holdTimer > Conductor.stepCrochet * 0.0011 * extrachar.singDuration && extrachar.animation.curAnim.name.startsWith('sing') && !extrachar.animation.curAnim.name.endsWith('miss'))
				{
					extrachar.dance();
				}
			}
	...
	public function cameraDisplacement(character:Character, mustHit:Bool)
	{
		...
		{
			if ((PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && mustHit) || (!PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection && !mustHit))
			{
		...
		switch(eventName) {
			...
			case 'Play Animation':
				...
				switch (value1)
				{
					...
					case 'laugh':
						if (char.curCharacter == 'ycr' || char.curCharacter == 'ycr-mad' || char.curCharacter == 'ycr-sonic-encore')
						{
							camGame.zoom += 0.03;
							camHUD.zoom += 0.06;
	...
	*/
	...
	private function onKeyPress(event:KeyboardEvent):Void
	{
		...
		if (!cpuControlled && startedCountdown && !paused && key > -1 && (FlxG.keys.checkStatus(eventKey, JUST_PRESSED) || ClientPrefs.controllerMode))
		{
			if(!boyfriend.stunned && generatedMusic && !endingSong)
			{
				...
				// Shadow Mario, why does this achievement exist?
				//									- Xero
	...
	private function keyShit():Void
	{
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			if (extrachar != null && extrachar.holdTimer > Conductor.stepCrochet * 0.0011 * extrachar.singDuration && extrachar.animation.curAnim.name.startsWith('sing') && !extrachar.animation.curAnim.name.endsWith('miss'))
			{
				extrachar.dance();
			}
		}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		if (daNote.noteType == 'ExtraChar Note')
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(note.hitCausesMiss) {
				...
				trace("Oh my god is that a " + note.noteType);

				switch(note.noteType){
					case 'Phantom Note': //ooooo spooky
						trace('lmaooo u hit it');
						healthDrop += 0.00025;
						dropTime = 10;
					case 'ExtraChar Note':
						trace('Pico!!! OHHH YEAHHH!!');
				}
			...
			if(!note.noAnimation) {
				...
				if(note.noteType == 'ExtraChar Note')
				{
					if (extrachar != null)
					{
						extrachar.playAnim(animToPlay + note.animSuffix, true);
	...
	override function stepHit()
	{
		...
		if (SONG.song.toLowerCase() == 'too-slow-encore')
		{
			switch (curStep)
			{
				...
				case 1888:
					supersuperZoomShit = false;
					FlxTween.tween(FlxG.camera, {zoom: 0.7}, 3, {ease: FlxEase.quadInOut});
					defaultCamZoom = 0.7;
				case 1925:
		...
		if (SONG.song.toLowerCase() == 'you-cant-run-encore')
		{
			switch (curStep)
			{
				...
				case 405:
					superZoomShit = true;
				case 520:
					FlxTween.tween(FlxG.camera, {zoom: 0.35}, 1.5, {ease: FlxEase.quadOut});
				case 527:
					superZoomShit = false;
		...
		*/
		...
		if (SONG.song.toLowerCase() == 'forestall-desire')
		{
			switch (curStep)
			{
				case 784:
					//--boyfriendGroup.remove(boyfriend);
					extracharGroup.remove(extrachar);
					//--dadGroup.remove(dad);
					//--boyfriend = new Boyfriend(boyfriend.x, boyfriend.y, 'bf-whisper');
					extrachar = new Boyfriend(extrachar.x, extrachar.y, 'pico-whisper');
					//--dad = new Character(dad.x, dad.y, 'requital-whisper');
					//--boyfriendGroup.add(boyfriend);
					extracharGroup.add(extrachar);
					//--dadGroup.add(dad);
					remove(lightsup);
					...
					playerStrums.forEach(function(spr:FlxSprite)
					{
						FlxTween.tween(spr, {alpha: 0}, 0.1, {ease: FlxEase.linear});
					});
					opponentStrums.forEach(function(spr:FlxSprite)
					{
						FlxTween.tween(spr, {alpha: 0}, 0.1, {ease: FlxEase.linear});
				...
				case 800:
					FlxTween.tween(camHUD, {alpha: 1}, 1.2,{ease: FlxEase.cubeInOut});
					FlxTween.tween(camGame, {alpha: 1}, 1.2,{ease: FlxEase.cubeInOut});
					FlxTween.tween(FlxG.camera, {zoom: 1}, 1, {ease: FlxEase.quadInOut});
					defaultCamZoom = 1;
				case 863:
					playerStrums.forEach(function(spr:FlxSprite)
					{
						FlxTween.tween(spr, {alpha: 0.75}, 1, {ease: FlxEase.linear});
					});
				case 935:
					playerStrums.forEach(function(spr:FlxSprite)
					{
						FlxTween.tween(spr, {alpha: 0}, 1, {ease: FlxEase.linear});
					});
				case 988:
					playerStrums.forEach(function(spr:FlxSprite)
					{
				...
				case 1056:
					opponentStrums.forEach(function(spr:FlxSprite)
					{
						FlxTween.tween(spr, {alpha: 1}, 0.1, {ease: FlxEase.linear});
					});
					playerStrums.forEach(function(spr:FlxSprite)
					{
						spr.x -= 645;
					...
					extracharGroup.remove(extrachar);
					//--dadGroup.remove(dad);
					//--boyfriend = new Boyfriend(boyfriend.x, boyfriend.y, 'bf-requital');
					extrachar = new Boyfriend(extrachar.x, extrachar.y, 'pico-requital');
					//--dad = new Character(dad.x, dad.y, 'requital');
					//--boyfriendGroup.add(boyfriend);
					extracharGroup.add(extrachar);
					//--dadGroup.add(dad);
					add(lightsup);
	...
	override function beatHit()
	{
		...
		if (extrachar != null && curBeat % extrachar.danceEveryNumBeats == 0 && extrachar.animation.curAnim != null && !extrachar.animation.curAnim.name.startsWith('sing') && !extrachar.stunned)
		{
			extrachar.dance();
		}
	...
	function cinematicBars(appear:Bool, time:Float)
	{
		if (appear)
		{
			add(topBar);
			add(bottomBar);
			FlxTween.tween(topBar, {y: 0}, time, {ease: FlxEase.quadOut});
			FlxTween.tween(bottomBar, {y: 550}, time, {ease: FlxEase.quadOut});
		}
		else
		{
			FlxTween.tween(topBar, {y: -170}, time, {ease: FlxEase.quadOut});
			FlxTween.tween(bottomBar, {y: 720}, time, {ease: FlxEase.quadOut, onComplete: function(fuckme:FlxTween)
			{
				remove(topBar);
				remove(bottomBar);
			}});
