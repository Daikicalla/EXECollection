...
class FreeplayState extends MusicBeatState // REWRITE FREEPLAY!?!?!? HELL YEA!!!!!
{
	...
	override function create()
	{
		...
		DiscordClient.changePresence("In the Main Menu", "Choosing in Freeplay");
		#end
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK)
		{
			...
			PlayState.isFreeplayMode = false;
				...
				boxgrp.forEach(function(thing:FlxSkewedSprite)
					{
						if (thing.ID == curSelected && thing.toString() == "char") 
						{
							switch(charArray[curSelected])
							{
								...
								case "starved":
									if (curSongSelected == 1) thing.loadGraphic(Paths.image('fpstuff/prey'));
									else thing.loadGraphic(Paths.image('fpstuff/fight-or-	ght'));
